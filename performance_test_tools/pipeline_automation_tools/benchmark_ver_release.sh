...
startTask() {
    task="$1"
    logInfo "${novadbVersionLongFormat} task begin: ${task}"
    resultPath="$2"
...
waitFinish() {
    ...
    logInfo "${novadbVersionLongFormat} task finished: $1"
    benchmarkPidList=()
}
...
runTest() {
    ...
    for valueSize in $(echo $valueSizeList | tr ',' '\n'); do
        for cmd in $(echo $cmdList | tr ',' '\n'); do
            ...
            getQPS ${startTimestamp} ${endTimestamp} ${novadbVersionShortFormat} ${cmd}
            decreaseLimit=''
            if [[ "$cmd" == "set" ]]; then
            ...
            outputReport "${cmd}测试曲线：<a href=\"${grafanaURL}${novadbVersionShortFormat}&from=${startTimestamp}000&to=${endTimestamp}000\">${grafanaURL}${novadbVersionShortFormat}&from=${startTimestamp}000&to=${endTimestamp}000</a>"
            if [[ ${qps} == "0.1" ||
                  ${P50} == "0.1" ||
                  ${P99} == "0.1" ||
                  ${P100} == "0.1" ||
            ...
            python3 writeTag.py ${cmd} ${novadbVersionShortFormat} $(date +%Y%m%d) ${qps} ${P50} ${P99} ${P100} ${AVG} ${mailfile} ${decreaseLimit} ${decreaseLimitP50} ${decreaseLimitP99} ${decreaseLimitP100} ${decreaseLimitPavg} ${shouldSave} ${compareToHistory} ${baselineVersion}
            sleep $interTime
        done
    done
    ...
    logInfo "${novadbVersionLongFormat} grafanaStartTimestamp:$grafanaStartTimestamp grafanaEndTimestamp:$grafanaEndTimestamp"
    ...
    outputReport "全过程监控<a href=\"${grafanaURL}${novadbVersionShortFormat}&from=${grafanaStartTimestamp}&to=${grafanaEndTimestamp}\">${grafanaURL}${novadbVersionShortFormat}&from=${grafanaStartTimestamp}&to=${grafanaEndTimestamp}</a>"
    ...
    python3 sendmail.py ${novadbVersionLongFormat}${emailTitleSubfix} ${mailfile} ${passid} ${token} ${mailURL} ${mailSender} ${sendmailgroup}
    mkdir -p mail
    mv ${mailfile} mail/
}
...
outputUsage() {
    echo "usage:"
    echo "    nohup ./benchmark_ver_release.sh novadbVersionLongFormat [testType, cmdList, valueSizeList, testTime]..."
    echo "example:"
    echo "    nohup ./benchmark_ver_release.sh novadbplus-2.4.0-rocksdb-v5.13.4-test multicmd set,get,sadd 64,1024 1800 pipeline set 128 1800 &"
    echo ""
...
main() {
    ...
    # get novadb version
    novadbVersionLongFormat=$1
    novadbVersionShortFormat=$(echo $1 | perl -p -e 's/\.//g' | perl -p -e 's/novadbplus-/cd/g' | perl -p -e 's/-rocksdb-/r/g' | perl -p -e 's/v//g' | perl -p -e 's/-test/test/g')
    shift
    logInfo "start novadbVersion: ${novadbVersionLongFormat} benchmark"
    ...
    logInfo "end novadbVersion: $novadbVersionLongFormat benchmark"
    logInfo "========end========"
}
