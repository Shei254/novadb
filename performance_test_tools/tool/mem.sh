...
do
    ...
    echo `ps axu |grep novadbplus|grep -v grep` >> $log
    free -m >> $log
    ...
    ${bin_dir}/redis-cli -h $ip -p $port $cli_pw novadbstat > $stat_dir/stat.$time_suffix
    ${bin_dir}/redis-cli -h $ip -p $port $cli_pw info > $stat_dir/info.$time_suffix

    start=`${bin_dir}/redis-cli -h $ip -p $port $cli_pw novadbstat|grep processed |awk -F " |:|," '{print $11}'`
    sleep $qps_sec
    end=`${bin_dir}/redis-cli -h $ip -p $port $cli_pw novadbstat|grep processed |awk -F " |:|," '{print $11}'`
