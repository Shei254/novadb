...
proc start_server {options {code undefined}} {
    ...
    # setup defaults
    set baseconfig "novadbplus.conf"
    set overrides {}
    set tags {}
    ...
    set config_file [tmpfile novadbplus.conf]
    set slave_cfg_file [tmpfile novadbplus.conf]
    ...
    set novadb_master_mem_log [tmpfile novadb_master_mem_$systemTime.log]
    set novadb_slave_mem_log [tmpfile novadb_slave_mem_$systemTime.log]

    if {$::valgrind} {
        exec valgrind --suppressions=src/valgrind.sup --show-reachable=no --show-possibly-lost=no --leak-check=full --soname-synonyms=somalloc=jemalloc ./build/bin/novadbplus $config_file > $stdout 2> $novadb_slave_mem_log &
        #exec valgrind --log-file=./valgrind.log --show-reachable=no --show-possibly-lost=no --leak-check=full --show-leak-kinds=all --soname-synonyms=somalloc=jemalloc ./build/bin/novadbplus $config_file > $stdout 2> $novadb_slave_mem_log &
    } else {
        exec ./build/bin/novadbplus $config_file > $stdout 2> $novadb_master_mem_log &
    }
    exec ./build/bin/novadbplus $slave_cfg_file > $stdout 2> $novadb_slave_mem_log &
    ...
    if {$code ne "undefined"} {
        ...
        set infofile "/novadbplus.WARNING"
        ...
        if {$break == 0} {
            ...
            # compare -addr1 {novadb ip:port} -addr2 {redis ip:port} cause only novadb support iterall.
