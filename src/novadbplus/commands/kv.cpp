...
#include "novadbplus/commands/dump.h"
...
class SetExCommand : public SetexGeneralCommand {
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    Expected<int64_t> eexpire = ::novadbplus::stoll(sess->getArgs()[2]);
    if (!eexpire.ok()) {
 ...
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    if (args.size() == 4 || args.size() == 5) {
      Expected<int64_t> estart = ::novadbplus::stoll(args[3]);
      if (!estart.ok()) {
        return estart.status();
      }
      start = estart.value();
      if (args.size() == 5) {
        Expected<int64_t> eend = ::novadbplus::stoll(args[4]);
        if (!eend.ok()) {
          return eend.status();
        }
        end = eend.value();
 ...
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    if (sess->getArgs().size() == 4) {
      Expected<int64_t> estart = ::novadbplus::stoll(sess->getArgs()[2]);
      Expected<int64_t> eend = ::novadbplus::stoll(sess->getArgs()[3]);
...
class CasCommand : public GetSetGeneral {
 public:
  ...
  Expected<RecordValue> newValueFromOld(
    ...
    Expected<uint64_t> ecas = ::novadbplus::stoul(sess->getArgs()[2]);
    if (!ecas.ok()) {
      return ecas.status();
    }
...
class SetRangeCommand : public GetSetGeneral {
 public:
  ...
  Expected<RecordValue> newValueFromOld(
    ...
    Expected<int64_t> eoffset = ::novadbplus::stoll(sess->getArgs()[2]);
    if (!eoffset.ok()) {
...
class SetBitCommand : public GetSetGeneral {
 public:
  ...
  Expected<RecordValue> newValueFromOld(
    Session* sess, const Expected<RecordValue>& oldValue) const {
    Expected<uint64_t> epos = ::novadbplus::stoul(sess->getArgs()[2]);
    if (!epos.ok()) {
      return epos.status();
    }
...
class IncrDecrGeneral : public GetSetGeneral {
 public:
  ...
  Expected<int64_t> sumIncr(const Expected<RecordValue>& esum,
    ...
    int64_t sum = 0;
    if (esum.ok()) {
      Expected<int64_t> val = ::novadbplus::stoll(esum.value().getValue());
      if (!val.ok()) {
  ...
  virtual Expected<std::string> run(Session* sess) {
    ...
    Expected<int64_t> val = ::novadbplus::stoll(rv.value().getValue());
    if (!val.ok()) {
      return val.status();
    }
...
class IncrbyfloatCommand : public GetSetGeneral {
 public:
  ...
  Expected<long double> sumIncr(const Expected<RecordValue>& esum,
    ...
    long double sum = 0;
    if (esum.ok()) {
      Expected<long double> val = ::novadbplus::stold(esum.value().getValue());
      if (!val.ok()) {
  ...
  virtual Expected<std::string> run(Session* sess) {
    ...
    Expected<long double> val = ::novadbplus::stold(rv.value().getValue());
    if (!val.ok()) {
      return val.status();
    }
    return Command::fmtBulk(::novadbplus::ldtos(val.value(), true));
  }

  Expected<RecordValue> newValueFromOld(
    Session* sess, const Expected<RecordValue>& oldValue) const {
    const std::string& val = sess->getArgs()[2];
    Expected<long double> eInc = ::novadbplus::stold(val);
    if (!eInc.ok()) {
      return eInc.status();
    }
    ...
    return RecordValue(::novadbplus::ldtos(newSum.value(), true),
                       type,
                       sess->getCtx()->getVersionEP(),
                       ttl,
                       oldValue);
...
class IncrexCommand : public IncrDecrGeneral {
 public:
  ...
  Expected<RecordValue> newValueFromOld(
    Session* sess, const Expected<RecordValue>& oldValue) const {
    const std::string& val = sess->getArgs()[2];
    Expected<int64_t> ettl = ::novadbplus::stoll(val);
    if (!ettl.ok()) {
      return ettl.status();
    }
 ...
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    for (size_t i = 2; i < args.size(); i++) {
      ...
      Expected<int32_t> eBits = novadbplus::stol(bitvals);
      ...
      Expected<int64_t> eOffset = novadbplus::stoll(offsetval);
      if (!eOffset.ok()) {
        return {ErrorCodes::ERR_PARSEPKT,
                "bit offset is not an integer or out of range"};
      }
      ...
      if (opcode != FieldOpType::BITFIELDOP_GET) {
        ...
        Expected<int64_t> eI64 = novadbplus::stoll(args[i]);
        if (!eI64.ok()) {
          return eI64.status();
        }
        i64 = eI64.value();
