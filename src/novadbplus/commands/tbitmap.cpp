...
#define GET_POS(param)                                       \
  auto epos = novadbplus::stoul(param);                      \
...
#define GET_START_END(param)                              \
  ...
  if (args.size() == param - 1 || args.size() == param) { \
    auto estart = ::novadbplus::stoll(args[param - 2]);   \
    RET_IF_ERR_EXPECTED(estart);                          \
    start = estart.value();                               \
    if (args.size() == param) {                           \
      auto eend = ::novadbplus::stoll(args[param - 1]);   \
...
Expected<int64_t> getBitOrOffset(const std::string& input, bool isBit) {
  ...
  int64_t val = -1;
  if (isBit) {
    auto ebit = novadbplus::stoul(input);
    ...
    auto epos = novadbplus::stoul(input);
    RET_IF_ERR_EXPECTED(epos);
    val = epos.value();
