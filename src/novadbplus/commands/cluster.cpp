...
#include "novadbplus/utils/base64.h"
...
class ClusterCommand : public Command {
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    const std::shared_ptr<novadbplus::ClusterState>& clusterState =
    ...
    if (arg1 == "setslot" && argSize >= 3) {
      ...
      if ((arg2 == "importing" || arg2 == "restart") && argSize >= 5) {
        ...
        for (auto& vs : vec) {
          if ((vs.find('{') != std::string::npos) &&
            ...
            Expected<int64_t> exptSlot = ::novadbplus::stoll(vs);
      ...
      uint64_t port, cport;
      auto& host = args[2];
      auto eport = ::novadbplus::stoul(args[3]);
      if (!eport.ok()) {
        return {ErrorCodes::ERR_CLUSTER,
      ...
      if (argSize == 5) {
        auto ecport = ::novadbplus::stoul(args[4]);
        if (!ecport.ok()) {
          return {ErrorCodes::ERR_CLUSTER,
      ...
      for (size_t i = 2; i < argSize; ++i) {
        if ((args[i].find('{') != std::string::npos) &&
          ...
          auto slotInfo = ::novadbplus::stoul(args[i]);
      ...
      auto eslot = ::novadbplus::stoul(args[2]);
      if (!eslot.ok()) {
      ...
      auto exptSlot = ::novadbplus::stoul(args[2]);
      if (!exptSlot.ok()) {
        return exptSlot.status();
      }
      uint32_t slot = exptSlot.value();
      auto ecount = ::novadbplus::stoul(args[3]);
      if (!ecount.ok()) {
      ...
      Expected<uint64_t> exptEpoch = ::novadbplus::stoul(args[2]);
      if (!exptEpoch.ok()) {
 ...
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    for (size_t n = 1; n < args.size();) {
      ...
      auto eTs = novadbplus::stoull(args[n + 1]);
      if (!eTs.ok()) {
        return eTs.status();
      }
      auto eVersion = novadbplus::stoull(args[n + 2]);
      if (!eVersion.ok()) {
