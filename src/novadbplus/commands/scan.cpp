...
#include "novadbplus/storage/record.h"
#include "novadbplus/storage/skiplist.h"
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    while (i < args.size()) {
      ...
      if (toLower(args[i]) == "count" && j >= 2) {
        Expected<uint64_t> ecnt = ::novadbplus::stoul(args[i + 1]);
    ...
    if (name == "zscanbyscore") {
      ...
      if (arr.value().size() == count + 1) {
        Command::fmtBulk(ss, ::novadbplus::dtos(arr.value().back().first));
        arr.value().pop_back();
      } else {
        Command::fmtNull(ss);
      }
      ...
      for (const auto& v : arr.value()) {
        ...
        Command::fmtBulk(ss, ::novadbplus::dtos(v.first));
      }
      return ss.str();
    }

    // cursor should be an integer
    auto ecursor = novadbplus::stoull(cursorArg);
    if (!ecursor.ok()) {
 ...
 public:
  ...
  Expected<std::string> genResult(Session* sess,
    ...
    for (const auto& v : rcds) {
      ...
      auto d = novadbplus::doubleDecode(v.getRecordValue().getValue());
...
class ZScanbyscoreCommand : public ScanGenericCommand {
 public:
  ...
  Expected<std::string> genResult(Session* sess,
    ...
    //     auto da =
    //     novadbplus::doubleDecode(a.getRecordValue().getValue()); auto db
    //     = novadbplus::doubleDecode(b.getRecordValue().getValue());
    //     if(!da.ok() || !db.ok()) {
    //         return false;
    //     }
    ...
    for (const auto& v : rcds) {
      ...
      auto d = novadbplus::doubleDecode(v.getRecordKey().getSecondaryKey());
      INVARIANT_D(d.ok());
      RET_IF_ERR_EXPECTED(d);
      Command::fmtBulk(ss, novadbplus::dtos(d.value()));
    }
...
class ScanCommand : public Command {
 public:
  ...
  static Status parseSessionArgs(const std::vector<std::string>& args,
    ...
    auto cursorCnt = novadbplus::stoul(args[1]);
    if (!cursorCnt.ok()) {
      return {ErrorCodes::ERR_PARSEOPT, "-ERR invalid cursor\r\n"};
    }
    *cursor = cursorCnt.value();
    ...
    while (index < args.size()) {
      if (toLower(args[index]) == "count" && index + 1 < args.size()) {
        auto cnt = novadbplus::stoul(args[index + 1]);
        ...
        *enableSlots = true;
        ...
        auto expNode = novadbplus::stoll(args[index + 1]);
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    /*
     * scan primary keys
     * NOTE(pecochen): for novadb, there is no need to make ScanCommand generic,
