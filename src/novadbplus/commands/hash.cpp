...
Expected<std::string> hincrfloatGeneric(Session* sess,
  ...
  if (getSubkeyExpt.ok()) {
    Expected<long double> val =
      ::novadbplus::stold(getSubkeyExpt.value().getValue());
    if (!val.ok()) {
  ...
  nowVal += inc;
  RecordValue newVal(
    ::novadbplus::ldtos(nowVal, true), RecordType::RT_HASH_ELE, -1);
  ...
  if (!exptCommit.ok()) {
    ...
    return Command::fmtBulk(novadbplus::ldtos(nowVal, true));
  }
}
...
Expected<std::string> hincrGeneric(Session* sess,
  ...
  if (getSubkeyExpt.ok()) {
    Expected<int64_t> val =
      ::novadbplus::stoll(getSubkeyExpt.value().getValue());
    if (!val.ok()) {
...
class HIncrByFloatCommand : public Command {
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    Expected<long double> inc = ::novadbplus::stold(val);
    if (!inc.ok()) {
      return inc.status();
    }
    ...
    // here maybe one more time io than the original novadb
...
Status hmcas(Session* sess,
  ...
  for (const auto& keyPos : uniqkeys) {
    ...
    Expected<uint64_t> eop = ::novadbplus::stoul(opstr);
    if (!eop.ok()) {
      return eop.status();
    }
    ...
    if (eop.value() == OPSET || (!exists && eop.value() == OPADD)) {
      ...
      Expected<int64_t> ev = ::novadbplus::stoll(existkvs[keyPos.first]);
      if (!ev.ok()) {
        return ev.status();
      }
      Expected<int64_t> ev1 = ::novadbplus::stoll(subargs[keyPos.second + 2]);
      if (!ev1.ok()) {
        return ev1.status();
      }
      RecordValue subrv(
...
class HMCasV2Command : public Command {
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    const std::string& key = args[1];
    Expected<uint64_t> ecmp = ::novadbplus::stoull(args[2]);
    if (!ecmp.ok()) {
      return ecmp.status();
    }
    Expected<int64_t> evsn = ::novadbplus::stoll(args[3]);
    if (!evsn.ok()) {
      return evsn.status();
    }
    Expected<int64_t> enewvsn = ::novadbplus::stoll(args[4]);
    if (!enewvsn.ok()) {
...
class HMCasCommand : public Command {
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    const std::string& key = args[1];
    Expected<uint64_t> ecmp = ::novadbplus::stoul(args[2]);
    if (!ecmp.ok()) {
      return ecmp.status();
    }
    Expected<int64_t> evsn = ::novadbplus::stoll(args[3]);
