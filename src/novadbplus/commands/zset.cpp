  ...
  for (const auto& subkey : subkeys) {
    ...
    if (eValue.status().code() == ErrorCodes::ERR_NOTFOUND) {
      ...
      Expected<double> oldScore =
        ::novadbplus::doubleDecode(eValue.value().getValue());
      if (!oldScore.ok()) {
...
Expected<std::string> genericZadd(Session* sess,
  ...
  if (incr) { /* ZINCRBY or INCR option. */
    if (processed)
      return Command::fmtBulk(::novadbplus::dtos(newScore));
    else
...
class ZRemByRangeGenericCommand : public Command {
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    if (_type == Type::RANK) {
      Expected<int64_t> estart = ::novadbplus::stoll(args[2]);
      if (!estart.ok()) {
        return estart.status();
      }
      Expected<int64_t> eend = ::novadbplus::stoll(args[3]);
      if (!eend.ok()) {
        return eend.status();
      }
      start = estart.value();
...
class ZIncrCommand : public Command {
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    const std::vector<std::string>& args = sess->getArgs();
    const std::string& key = args[1];
    Expected<double> score = ::novadbplus::stod(args[2]);
    if (!score.ok()) {
      return score.status();
    }
    ...
    if (args.size() > 4) {
      ...
      while (remaining) {
        if (remaining >= 1 && toLower(args[pos]) == "withscores") {
          ...
          Expected<int64_t> eoffset = ::novadbplus::stoll(args[pos + 1]);
          if (!eoffset.ok()) {
            return eoffset.status();
          }
          offset = (uint64_t)eoffset.value();
          Expected<int64_t> elimit = ::novadbplus::stoll(args[pos + 2]);
          if (!elimit.ok()) {
...
class ZScoreCommand : public Command {
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    return Command::fmtBulk(::novadbplus::dtos(oldScore.value()));
  }
} zscoreCmd;
 ...
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    for (; i < args.size(); i += 2) {
      ...
      Expected<double> score = ::novadbplus::stod(args[i]);
      if (!score.ok()) {
        return score.status();
      }
...
class ZUnionInterGenericCommand : public Command {
 public:
  ...
  std::vector<int> getKeysFromCommand(
    ...
    Expected<int32_t> expLen = novadbplus::stol(argv[2]);
    if (!expLen.ok()) {
  ...
  Expected<std::string> run(Session* sess) final {
    const std::vector<std::string>& args = sess->getArgs();
    Expected<int32_t> expLen = novadbplus::stol(args[2]);
    if (!expLen.ok()) {
      ...
      while (remaining) {
        if (remaining >= len + 1 && !::strcasecmp(args[j].c_str(), "weights")) {
          ...
          for (int i = 0; i < len; i++, j++, remaining--) {
            Expected<double> expScore = novadbplus::stod(args[j]);
        ...
        for (auto iter = scoreMap.begin(); iter != scoreMap.end();) {
          ...
          if (keyType == RecordType::RT_ZSET_META) {
            Expected<double> eScore =
              novadbplus::doubleDecode(eVal.value().getValue());
            if (!eScore.ok()) {
