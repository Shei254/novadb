...
#ifndef WIN32
#ifdef novadb_JEMALLOC
#include "jemalloc/jemalloc.h"
#endif  // !novadb_JEMALLOC
#endif  // !WIN32
...
class IterAllKeysCommand : public Command {
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    const std::vector<std::string>& args = sess->getArgs();
    Expected<uint64_t> estoreId = ::novadbplus::stoul(args[1]);
    if (!estoreId.ok()) {
      return estoreId.status();
    }
    Expected<uint64_t> ebatchSize = ::novadbplus::stoul(args[3]);
    if (!ebatchSize.ok()) {
    ...
    for (const auto& o : result) {
      ...
      switch (t) {
        ...
        case RecordType::RT_LIST_ELE: {
          ...
          auto expIdx = novadbplus::stoul(o.getRecordKey().getSecondaryKey());
          if (!expIdx.ok()) {
        ...
        case RecordType::RT_ZSET_H_ELE: {
          ...
          Command::fmtBulk(ss, o.getRecordKey().getPrimaryKey());
          Command::fmtBulk(ss, o.getRecordKey().getSecondaryKey());
          auto d = novadbplus::doubleDecode(o.getRecordValue().getValue());
          if (!d.ok()) {
            return d.status();
          }
          Command::fmtBulk(ss, novadbplus::dtos(d.value()));
          break;
        }
...
class RocksPropertyCommand : public Command {
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    if (sess->getArgs().size() > 2) {
      auto e = ::novadbplus::stoll(args[2]);
      if (!e.ok()) {
        return e.status();
      }
      first = e.value();
...
class BinlogTimeCommand : public Command {
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    const std::vector<std::string>& args = sess->getArgs();
    Expected<uint64_t> storeId = ::novadbplus::stoul(args[1]);
    if (!storeId.ok()) {
    ...
    Expected<uint64_t> binlogId = ::novadbplus::stoul(args[2]);
    if (!binlogId.ok()) {
...
class RecycleStatusCommand : public Command {
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    const auto& args = sess->getArgs();
    auto storeId = ::novadbplus::stoul(args[1]);
    if (!storeId.ok()) {
      return storeId.status();
    }
 ...
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    if (kvstore == "all") {
      ...
      Expected<uint64_t> exptStoreId = ::novadbplus::stoul(kvstore.c_str());
      if (!exptStoreId.ok()) {
...
class novadbstatCommand : public Command {
 public:
  novadbstatCommand() : Command("novadbstat", "as") {}
...
class ClientCommand : public Command {
 public:
    ...
    if (args.size() == 3) {
      ...
      while (i < args.size()) {
        ...
        if (args[i] == "id" && moreargs) {
          Expected<uint64_t> eid = ::novadbplus::stoul(args[i + 1]);
          if (!eid.ok()) {
            return eid.status();
          }
          id = eid.value();
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    auto arg1 = novadbplus::toLower(args[1]);
...
#endif
      ss << "# Server\r\n"
         << "redis_version:" << getnovadbPlusVersion() << "\r\n"
         << "redis_git_sha1:" << novadbPLUS_GIT_SHA1 << "\r\n"
         << "redis_git_dirty:" << novadbPLUS_GIT_DIRTY << "\r\n"
    ...
    if (section == "rocksdbstats") {
      ...
      for (uint64_t i = 0; i < server->getKVStoreCount(); ++i) {
        ...
        for (auto one : v) {
          ...
          auto e = novadbplus::stoul(key_value[1]);
          if (!e.ok()) {
            continue;
          }
 ...
 public:
  ...
  Status configSetCommand(Session* sess) {
    ...
    if (configName == "session") {
      ...
      if (toLower(args[3]) == "novadb_protocol_extend") {
...
class FlushOneStoreCommand : public FlushGeneric {
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    Expected<uint64_t> storeId = ::novadbplus::stoul(sess->getArgs()[1]);
    if (!storeId.ok()) {
      return storeId.status();
    }
...
class setInStoreCommand : public Command {
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    Expected<uint64_t> eid = ::novadbplus::stoul(args[1]);
    if (!eid.ok()) {
      return eid.status();
    }
...
class SyncVersionCommand : public Command {
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    auto eTs = novadbplus::stoull(args[2]);
    RET_IF_ERR(eTs.status());
    auto eVersion = novadbplus::stoull(args[3]);
 ...
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    if (toLower(args[1]) == "len") {
      ...
      uint64_t count_num;
      if (args.size() == 3) {
        Expected<uint64_t> count = novadbplus::stoull(args[2]);
        if (!count.ok())
...
class reshapeCommand : public Command {
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    if (args.size() == 2) {
      auto expStoreId = novadbplus::stoull(args[1]);
      if (!expStoreId.ok()) {
...
class compactRangeCommand : public Command {
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    auto estart = novadbplus::stoull(args[2]);
    RET_IF_ERR_EXPECTED(estart);
    uint64_t start = estart.value();

    auto eend = novadbplus::stoull(args[3]);
    ...
    if (args.size() > 4) {
      auto eStoreId = novadbplus::stoull(args[4]);
...
class deleteFilesInRangeGenericCommand : public Command {
 public:
  ...
  Expected<std::string> deleteOnDataCF(Session* sess) {
    ...
    auto expStart = ::novadbplus::stoull(args[2]);
    RET_IF_ERR_EXPECTED(expStart);
    uint64_t start = expStart.value();

    auto expEnd = ::novadbplus::stoull(args[3]);
    ...
    if (args.size() > 4) {
      ...
      auto expStoreID = ::novadbplus::stoull(args[4]);
...
class deleteSlotsCommand : public Command {
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    auto eBeginChunkid = novadbplus::stoul(args[1]);
    if (!eBeginChunkid.ok()) {
      return eBeginChunkid.status();
    }
    uint32_t beginChunkid = eBeginChunkid.value();

    auto eEndChunkid = novadbplus::stoul(args[2]);
    if (!eEndChunkid.ok()) {
...
class novadbadminCommand : public Command {
 public:
  novadbadminCommand() : Command("novadbadmin", "lats") {}
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    if (operation == "sleep" || operation == "lockdb") {
      ...
      auto time = novadbplus::stoull(args[2]);
      if (!time.ok()) {
        return time.status();
      }
...
class DExecCommand : public Command {
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    auto eid = novadbplus::stoll(args[1]);
    if (!eid.ok()) {
      return eid.status();
    }
    ...
    for (size_t i = 2; i < args.size(); i += 2) {
      if (toLower(args[i]) == "storeid" && i + 1 < args.size()) {
        if (args[i + 1] == "*") {
          ...
          auto expStoreId = novadbplus::stoul(args[i + 1]);
          RET_IF_ERR_EXPECTED(expStoreId);
          storeId = expStoreId.value();
          allStore = false;
 ...
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    return {ErrorCodes::ERR_INTERNAL,
            ...
            "Try to rebuild novadb with correct params."};
#endif  // !novadb_JEMALLOC
  }
} jeprofCommand;
