...
Expected<std::vector<byte>> Serializer::dump(bool prefixVer) {
  ...
  if (prefixVer) {
    ...
    /* special case for keys that directly generated by novadbplus iteself
     */
    if (revision == UINT64_MAX) {
      revision = 0;
      LOG(WARNING) << "redis send dumpx command to an"
                   << "novadb-generated key: " << _key;
    }
 ...
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    for (const auto& i : index) {
      auto expDbid = novadbplus::stoul(args[i - 1]);
      if (!expDbid.ok()) {
...
class RestoreCommand : public Command {
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    Expected<int64_t> expttl = novadbplus::stoll(sttl);
    if (!expttl.ok()) {
...
class RestoreMetaCommand : public Command {
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    auto eSlotId = novadbplus::stoul(sess->getArgs()[1]);
    if (!eSlotId.ok()) {
    ...
    if (sess->getArgs().size() > 2) {
      ...
      auto arg3 = novadbplus::stoul(sess->getArgs()[3]);
      if (!arg3.ok()) {
        return arg3.status();
      }
      ...
      auto arg4 = novadbplus::stoul(sess->getArgs()[4]);
      if (!arg4.ok()) {
        return arg4.status();
      }
...
Expected<std::string> recordList2Aof(const std::list<Record>& list) {
  ...
  switch (type) {
    ...
    case novadbplus::RecordType::RT_LIST_ELE:
      Command::fmtMultiBulkLen(ss, 2 + list.size());
      Command::fmtBulk(ss, "RPUSH");
      Command::fmtBulk(ss, key);
      break;

    case novadbplus::RecordType::RT_HASH_ELE:
      Command::fmtMultiBulkLen(ss, 2 + list.size() * 2);
      Command::fmtBulk(ss, "HMSET");
      Command::fmtBulk(ss, key);
      break;

    case novadbplus::RecordType::RT_SET_ELE:
      Command::fmtMultiBulkLen(ss, 2 + list.size());
      Command::fmtBulk(ss, "SADD");
      Command::fmtBulk(ss, key);
      break;

    case novadbplus::RecordType::RT_ZSET_H_ELE:
      Command::fmtMultiBulkLen(ss, 2 + list.size() * 2);
    ...
    switch (type) {
      ...
      case novadbplus::RecordType::RT_ZSET_H_ELE: {
        Expected<double> oldScore =
          ::novadbplus::doubleDecode(rtValue.getValue());
        if (!oldScore.ok()) {
          return oldScore.status();
        }
        Command::fmtBulk(ss, ::novadbplus::dtos(oldScore.value()));
      ...
      case novadbplus::RecordType::RT_TBITMAP_ELE: {
        auto eBitmapId = novadbplus::stoull(rtKey.getSecondaryKey());
        if (!eBitmapId.ok()) {
...
class IncrMetaCommand : public Command {
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    auto eStartRevision = novadbplus::stoul(args[1]);
    auto eEndRevision = novadbplus::stoul(args[2]);
