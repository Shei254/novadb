...
class ToggleIncrSyncCommand : public Command {
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    Expected<uint64_t> state = ::novadbplus::stoul(sess->getArgs()[1]);
    if (!state.ok()) {
      return state.status();
    }
...
class ApplyBinlogsGeneric : public Command {
 ...
 public:
  ...
  static Status runMigrate(Session* sess,
    ...
    Expected<int64_t> etype = ::novadbplus::stoll(splits[0]);
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    uint32_t storeId;
    Expected<uint64_t> exptStoreId = ::novadbplus::stoul(args[1]);
    if (!exptStoreId.ok()) {
      return exptStoreId.status();
    }
    ...
    uint64_t binlogCnt;
    auto exptCnt = ::novadbplus::stoul(args[3]);
    if (!exptCnt.ok()) {
      return exptCnt.status();
    }
    binlogCnt = exptCnt.value();

    auto eflag = ::novadbplus::stoul(args[4]);
    if (!eflag.ok()) {
...
class RestoreBinlogCommandV2 : public Command {
 public:
  ...
  Status runMigrate(Session* sess,
    ...
    Expected<uint64_t> cmdStoreId = ::novadbplus::stoul(splits[1]);
    if (!cmdStoreId.ok()) {
 ...
 public:
  ...
  Expected<std::string> run(Session* sess) final {
    ...
    if (sess->getArgs().size() > 2) {
      auto eBinlogts = ::novadbplus::stoul(args[2]);
    ...
    Status s =
      replMgr->applyRepllogV2(sess, storeId, "", ::novadbplus::ultos(binlogTs));
    if (!s.ok()) {
      return s;
    }
    return Command::fmtOK();
