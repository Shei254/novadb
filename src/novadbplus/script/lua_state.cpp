...
extern "C" {
#include "novadbplus/script/sha1.h"
}
...
int string2ll(const char* s, size_t slen, int64_t* value) {
  auto expt = ::novadbplus::stoll(std::string(s, slen));
  if (!expt.ok()) {
...
void LuaState::pushThisToLua(lua_State* lua) {
  ...
  lua_pushstring(lua, novadbplus::ultos(p).c_str());
...
Expected<std::string> LuaState::evalGenericCommand(Session* sess, int evalsha) {
  ...
  const auto& eNumkeys = ::novadbplus::stoll(args[2]);
  if (!eNumkeys.ok()) {
  ...
  if (lua_isnil(_lua, -1)) {
     ...
     * (New in novadb) If this is an EVALSHA call, we will get script content
     * from kvstore as novadb stores all lua scripts received persistently. */
