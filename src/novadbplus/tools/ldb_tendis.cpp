...
void TScanCommand::Help(std::string& ret) {
  ret.append(
    "\
usage:\n\
  /path/to/ldb_novadb --db=path --column_family=cf tscan [option]\n\
\n\
  ldb_novadb options:\n\
    --db=path\n\
          rocksdb dir path\n\
...
static void PrintHelp(const LDBOptions& ldb_options,
                      ...
                      bool to_stderr) {
  std::string ret;
  ret.append("ldb_novadb - novadbplus Tool");
  ret.append("\n\n");
...
int main(int argc, char** argv) {
  ...
  if (argc <= 2) {
    if (argc <= 1) {
      ...
      printf("ldb_novadb from RocksDB %d.%d.%d\n",
             ROCKSDB_MAJOR,
             ROCKSDB_MINOR,
             ROCKSDB_PATCH);
      return 0;
