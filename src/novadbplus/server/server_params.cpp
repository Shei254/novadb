...
std::shared_ptr<novadbplus::ServerParams> gParams;
...
ServerParams::ServerParams() {
  ...
  REGISTER_VARS_DIFF_NAME_DYNAMIC("novadb-latency-limit", novadbLatencyLimit);
...
Status ServerParams::setRocksOption(const std::string& argname,
                                    const std::string& value) {
  std::string errinfo;
  auto argArray = novadbplus::stringSplit(argname, ".");
  ...
  if (startup) {
    ...
    if (iter == _mapServerParams.end()) {
      ...
      if (argname == "rocks.latency-limit") {
        auto ed = novadbplus::stoll(value);
        if (!ed.ok()) {
          errinfo = "invalid rocksdb options:" + argname + " value:" + value +
            " " + ed.status().toString();
          return {ErrorCodes::ERR_PARSEOPT, errinfo};
