...
#include "novadbplus/server/index_manager.h"
...
std::shared_ptr<ServerParams> GenServerParams() {
  ...
  if (FLAGS_db != nullptr) {
    ...
    myfile << "pidfile ./" << FLAGS_db << "/novadbplus.pid\n";
  } else {
    myfile << "logdir ./log\n";
    myfile << "dir ./db\n";
    myfile << "dumpdir ./dump\n";
    myfile << "pidfile ./novadbplus.pid\n";
  }
...
void printVersionInfo() {
  // db_stress tool, novadbplus v=2.4.3-rocksdb-v6.23.3
  //   sha=f4600de9 dirty=23 build=tlinux-1650438040
  std::cout << "db_stress tool, novadbplus v=" << getnovadbPlusVersion()
...
void printHelpInfo() {
  /*
  db_stress -- Benchmark Tool for novadbplus
  ...
  1 = on
      ...
      --rocksTransactionMode=mode   txn mode for novadbplus.
  ...
  */
  std::cout << "db_stress -- Benchmark Tool for novadbplus" << std::endl;
  ...
  std::cout << "    --rocksTransactionMode=mode   "
            << "txn mode for novadbplus." << std::endl;
  std::cout << "                                  "
...
int main(int argc, char** argv) {
  ...
  novadbplus::g_env = rocksdb::Env::Default();

  // Choose a location for the test database if none given with --db=<path>
  if (FLAGS_db == nullptr) {
    novadbplus::g_env->GetTestDirectory(&default_db_path);
