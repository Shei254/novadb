...
std::string Session::getSessionCmd() {
  ...
  if ((arg1 == "expire" || arg1 == "pexpire" || arg1 == "expireat" ||
      ...
      args.size() == 3) {
    std::stringstream ss;
    auto expt = ::novadbplus::stoll(args[2]);
    if (!expt.ok()) {
    ...
    auto expt = ::novadbplus::stoul(args[2]);
    if (!expt.ok()) {
  ...
  for (auto it = args.begin(); it != args.end();) {
    ...
    if ((arg == "ex" || arg == "px") && (i == 4 || i == 5)) {
      auto expt = ::novadbplus::stoul(*(it + 1));
      if (!expt.ok()) {
        LOG(ERROR) << "get EX|PX command time fail" << expt.status().toString();
        return "\"\"";
      }
...
Status Session::processExtendProtocol() {
  ...
  // cmd key timestamp version novadbex
  if (_args.size() < 4) {
  ...
  if (toLower(_args[i]) == "v1") {
    auto v = novadbplus::stoull(_args[--i]);
    if (!v.ok()) {
      return v.status();
    }
    uint64_t version = v.value();
    v = novadbplus::stoull(_args[--i]);
    if (!v.ok()) {
      return v.status();
    }
