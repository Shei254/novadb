...
#include "novadbplus/lock/lock.h"
#include "novadbplus/network/latency_record.h"
...
void ServerEntry::logError(const Status& s, Session* sess) {
  ...
  ss << ", error:" << novadbplus::catRepr(s.toString());
...
bool ServerEntry::processRequest(Session* sess) {
  ...
  if (expCmd.value()->isBgCmd()) {
    ...
    if (expCmdName == "fullsync") {
      ...
      INVARIANT(ns != nullptr);
      std::vector<std::string> args = ns->getArgs();
      auto esNum = ::novadbplus::stoul(args[4]);
      if (!esNum.ok())
      ...
      auto val = novadbplus::stoul(args[3]);
...
Status ServerEntry::setStoreMode(PStore store, KVStore::StoreMode mode) {
  ...
  auto storeId = novadbplus::stoul(store->dbId());
  if (!storeId.ok()) {
