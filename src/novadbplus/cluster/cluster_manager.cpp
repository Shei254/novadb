...
auto ClusterNode::parseClusterNodesInfo(const std::string& info)
  ...
  auto slotsMeta = novadbplus::stringSplit(info, ",");

  for (const auto& v : slotsMeta) {
    auto slotPair = novadbplus::stringSplit(v, "-");
    if (slotPair.size() == 1) {
      auto slot = novadbplus::stol(slotPair[0]);
      ...
      auto slotStart = novadbplus::stol(slotPair[0]);
      auto slotEnd = novadbplus::stol(slotPair[1]);
...
ClusterManager::ClusterManager(const std::shared_ptr<ServerEntry>& svr)
  ...
  std::shared_ptr<ClusterState> state =
    std::make_shared<novadbplus::ClusterState>(_svr);
