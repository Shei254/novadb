...
func testVersion(versions []string) {
	for _, v := range versions {
		...
		// copy target version novadbplus
		r := exec.Command("/bin/sh", "-c", "cp /data/binary/novadbplus-"+v+".tgz _version_test_/; cd _version_test_/; tar -zxvf novadbplus-"+v+".tgz;")
		_, err := r.CombinedOutput()
		if err != nil {
			log.Fatalf("get target version novadbplus failed! check if /data/binary exists.")
		}

		clusterNodeNum := 3
		servers, predixy, _ := startCluster("127.0.0.1", 30000, clusterNodeNum, map[string]string{"timeoutSecBinlogWaitRsp": "3", "minBinlogKeepSec": "3600", "keysDefaultLimit": "10000000"}, util.GetCurrentDirectory()+"/_version_test_", "_version_test_/novadbplus-"+v+"/bin/novadbplus")
		...
		for i := clusterNodeNum; i < clusterNodeNum*2; i++ {
			...
			// In tsan, novadb will taken 2~4 seconds to perform failover.
			time.Sleep(5 * time.Second)
		}
		...
		if oldMasterDbsize != totalKeyNumber {
			log.Fatalf("dbsize %v in novadb not equal to total input data %v.", oldMasterDbsize, totalKeyNumber)
		}
