...
func (s *RedisServer) Setup(valgrind bool, cfgArgs *map[string]string) error {
	...
	cfg = cfg + fmt.Sprintf("pidfile %s/novadbplus.pid\n", s.Path)
	if cfgArgs != nil {
...
func (s *RedisServer) Start(valgrind bool, cfgFilePath string) error {
	...
	if valgrind {
		...
		cmd := fmt.Sprintf("nohup ./valgrind --tool=memcheck --leak-check=full %s %s >valgrind_novadb_%d.log 2>&1 &", binPath, cfgFilePath, s.Port)
		args = append(args, cmd)
		inShell := true
		StartProcess(args, []string{}, fmt.Sprintf("%s/novadbplus.pid", s.Path), 10*time.Second, inShell, CheckPidFile)
	} else {
		log.Infof("start normal %d", s.Port)
		//args = append(args, "../../../build/bin/novadbplus", cfgFilePath)
		//inShell := false
		//_, err := StartProcess(args, []string{}, fmt.Sprintf("%s/novadbplus.pid", s.Path), 10*time.Second, inShell, CheckPidFile)
