...
if [[ "$1" == "versiontest" ]]; then
    processNameList=(novadbplus predixy)
    ...
    processNameList=(novadbplus predixy redis-server redis-sync checkdts)
    baseCmd="${baseCmd} | grep -v _version_test_"
fi

for pName in ${processNameList[@]}; do
    function getpids() {
        # only novadbplus and predixy contains 'integrate_test' in process command
        localCmd="${baseCmd}"
        if [[ "${pName}" == "novadbplus" || "${pName}" == "predixy" ]]; then
