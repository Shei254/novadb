...
bool SessionCtx::verifyVersion(uint64_t keyVersion) {
  ...
  if (_version == UINT64_MAX) {
    // isolate novadb cmd cannot modify value of novadb with cache.
...
std::string SessionCtx::generateLockRecordLogIfNeeded(LockLatencyType type) {
  if (!_session->getServerEntry() ||
      _session->getServerEntry()->getParams()->novadbLatencyLimit == 0 ||
      _lockRecord[type]._totalTimeAcquireLock <
        _session->getServerEntry()->getParams()->novadbLatencyLimit) {
    return "";
  }
