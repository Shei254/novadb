...
std::shared_ptr<ServerParams> makeServerParam(
  ...
  if (dir != "") {
    ...
    myfile << "pidfile ./" << dir << "/novadbplus.pid\n";
  } else {
    myfile << "logdir ./log\n";
    myfile << "dir ./db\n";
    myfile << "dumpdir ./dump\n";
...
void WorkLoad::lockDb(mstime_t locktime) {
  _session->setArgs({"novadbadmin", "lockdb", std::to_string(locktime)});
  auto expect = Command::runSessionCmd(_session.get());
  EXPECT_TRUE(expect.ok());
}

void WorkLoad::sleep(mstime_t locktime) {
  _session->setArgs({"novadbadmin", "sleep", std::to_string(locktime)});
  auto expect = Command::runSessionCmd(_session.get());
  EXPECT_TRUE(expect.ok());
}
...
void testHash2(std::shared_ptr<ServerEntry> svr) {
  ...
  sess.setArgs(
    {"hincrbyfloat", "testkey", "testsubkey", novadbplus::ldtos(pi, true)});
  expect = Command::runSessionCmd(&sess);
  EXPECT_TRUE(expect.ok());
  std::string result = novadbplus::ldtos(floatSum, true);
